...
namespace SocialNetwork.Data
{
    public class SocialNetworkDbContext : DbContext
    {
        private const string ConnectionString = 
            "Server=HAMILTON\\SQLEXPRESS01;Database=SocialNetwork;User Id=kristianaStefanova95;Password=337876Kri.;Encrypt=False;TrustServerCertificate=True;Encrypt=False;";
        ...
        {
            modelBuilder.Entity<UserConversation>(uc =>
            {
                uc.HasKey(uc => new { uc.UserId, uc.ConversationId });

            });

            modelBuilder.Entity<Friendship>(f =>
            {
                f.HasKey(f => new { f.UserOneId, f.UserTwoId });

                f.HasOne(f => f.UserOne)
                    .WithMany(u => u.FriendshipsRequested)
                    .HasForeignKey(f => f.UserOneId)
                    .OnDelete(DeleteBehavior.ClientCascade);

                f.HasOne(f => f.UserTwo)
